...
void InitNet(struct vocabulary *wv, struct vocabulary *cv) {
   long long a, b;
   a = posix_memalign((void **)&syn1neg, 128, (long long)wv->vocab_size * layer1_size * sizeof(real));
   if (syn1neg == NULL) {printf("Memory allocation failed\n"); exit(1);}
   for (b = 0; b < layer1_size; b++) 
      for (a = 0; a < wv->vocab_size; a++)
         syn1neg[a * layer1_size + b] = (rand() / (real)RAND_MAX - 0.5) / layer1_size;

   a = posix_memalign((void **)&syn0, 128, (long long)cv->vocab_size * layer1_size * sizeof(real));
...
void *TrainModelThread(void *id) {
   int wrdi = -1;
   int *ctxiList = (int *)calloc(100, sizeof(int)); //hack, window size must <= 100
  long long d, i;
  ...
  for (iter=0; iter < numiters; ++iter) {
     ...
     while (1) { //HERE @@@
        ...
        ctxiListLength = 0;
        while(1) {
            int t = ReadWordIndex(cv, fi);
            //printf("t=%d\n", t);
            if(t==-2)
                break;
            if(t<0)
                continue;
           real ran = (sqrt(cv->vocab[t].cn / (sample * cv->word_count)) + 1) * (sample * cv->word_count) / cv->vocab[t].cn;
           next_random = next_random * (unsigned long long)25214903917 + 11;
           if (ran < (next_random & 0xFFFF) / (real)65536) continue;

            ctxiList[ctxiListLength] = t;
            if(ctxiListLength <= 98)
                ctxiListLength++;
        }
       ...
       //
        ...
        for(i = 0; i < ctxiListLength; i++)  {
            if(ctxiList[i] < 0) {
                printf("error ctxiList[i] < 0!! \n");   
                //fflush(stdout);
                continue;
            }
            for (c = 0; c < layer1_size; c++) { 
                neu1[c] += syn0[c + ctxiList[i] * layer1_size];
            }
        }
	for(c = 0; c < layer1_size; c++) {
		neu1[c] /= ctxiListLength;
	}
        //fread(&wrdi, 4, 1, fi);
        //fread(&ctxi, 4, 1, fi);
        // NEGATIVE SAMPLING
        //printf("ctxiListLength=%d\n", ctxiListLength);
        for (d = 0; d < negative + 1; d++) {
           if (d == 0) {
              ...
              if (target == 0) target = next_random % (wv->vocab_size - 1) + 1;
        ...
        for(i = 0; i < ctxiListLength; i++)
            for (c = 0; c < layer1_size; c++)
                syn0[c + ctxiList[i] * layer1_size] += neu1e[c];
     }
  }
  fclose(fi);
  free(neu1);
...
void TrainModel() {
  ...
  wv = ReadVocab(wvocab_file, min_count);
  cv = ReadVocab(cvocab_file, min_count);
  InitNet(wv, cv);
  InitUnigramTable(wv);
  start = clock();
  ...
  if (classes == 0) {
    ...
    if (dumpcv_file[0] != 0) {
        ...
        int real_vocab_size = 0;
        for (a = 0; a < cv->vocab_size; a++) {
           if(strstr(cv->vocab[a].word, "#"))
                continue;
           real_vocab_size++;
        }
        fprintf(fo2, "%d %d\n", real_vocab_size, layer1_size);
        for (a = 0; a < cv->vocab_size; a++) {
           if(strstr(cv->vocab[a].word, "#"))
                continue;
