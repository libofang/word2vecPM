...
int get_cooccurrence() {
    ...
    if (verbose > 1) fprintf(stderr, "Reading vocab_word from file \"%s\"...", vocab_file);
	...
	ReadWord(str, fid, MAX_STRING_LENGTH - 2);
        if (feof(fid)) break;
        if(str[0] == '@' && str[1] == 0) {
            continue;
        }
        htmp = hashsearch(vocab_hash, str);
        if (htmp == NULL) w1 = -1; // Skip out-of-vocabulary words
        else
            w1 = htmp->id; // Target word (frequency rank)
        ...
        htmp = hashsearch(vocab_hash, str);
        if (htmp == NULL) w2 = -1; // Skip out-of-vocabulary words
	else
...
//	fprintf(stderr, "bf=%d %d %s\n", w1, w2, str);
        
	if(str[0] == '@' && str[1] == 0 && w1 != -1 && w2 != -1)
...
int main(int argc, char **argv) {
    ...
    ddir = malloc(sizeof(char) * MAX_STRING_LENGTH);  
    if (argc == 1) {
    ...
    if ((i = find_arg((char *)"-ddir", argc, argv)) > 0) strcpy(ddir, argv[i + 1]);
    if ((i = find_arg((char *)"-vocab_file", argc, argv)) > 0) strcpy(vocab_file, argv[i + 1]); 
